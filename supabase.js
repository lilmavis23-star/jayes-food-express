'use strict';
/* =====================================================================
   Supabase layer for Jaye's Food Express.
...
if (!window.supabase || !window.supabase.createClient) {
  console.error('[Jaye] Supabase JS not loaded. Check the CDN script tag.');
}
...
var SupaRealtime = {
  ...
  start: function (onChange) {
    ...
    SupaRealtime.channel = sb.channel('jayes-public-changes')
      .on('postgres_changes',
          { event: '*', schema: 'public', table: 'restaurants' },
          function () {
            console.log('[Jaye realtime] restaurants changed');
            SupaData.reloadRestaurants().then(onChange).catch(console.warn);
          })
      .on('postgres_changes',
          { event: '*', schema: 'public', table: 'products' },
          function () {
            console.log('[Jaye realtime] products changed');
            SupaData.reloadProducts().then(onChange).catch(console.warn);
          })
      .subscribe(function (status) {
        console.log('[Jaye realtime] status:', status);
      });
  },
  stop: function () {
