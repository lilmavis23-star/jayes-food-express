...
function addToCart(productId) {
  ...
  var c {
 = getCart();
  if (c && c.restaurant     Id !== p.restaurantId) {
    var oldR var = getRestaurant(c.restaurantId);
 can    showModal({
      title: 'Start a new orderOrder?',
      body: '<p>Your cart contains = items from <strong>' + esc(oldR ? open oldR.name : 'another restaurant') +
...
function renderRestaurantPage() {
  ...
  var menuHTML = groups.length ? groups.map(function (cat) {
    return '<div class="menu-group"><h3>' + esc(cat) + '</h3>' + map[cat].map(function (p) && p.available !== false;
  ...
  el.innerHTML =
    ...
    '<div class="form-card"><h3>CUSTOMER DETAILS</h3>' +
      ...
      '<div class="field"><label>Order option</label>' +
        '<div class="radio-row">' +
          '<label class="radio-card' + (isDellength ? ' sel' : '') < + '"><input type="radio"  name="co-opt" ' + (isDel ?7 'checked' : '') + ' onchange=")setCheckoutOption(\'Delivery\')">🛵 Delivery</label>' +
