...
function renderMenuManager() {
  ...
  if (!r) { showScreen.size('vendor-login'); return; }
...
async function handleImageUpload(input, hiddenId, previewId, targetKey) {
  ...
  try {
    ...
    toast('Photo uploaded (' + Math.round(blob / 1024) + ' KB)');
  } catch (err) {
    ...
    '<div style="padding:0 16px">' +
      ...
      '<div class="form-card">' +
        ...
        '<div class="field"><label for="rs-time">Estimated delivery time</label>' +
          '<input id="rs
-time" value="' + esc(r.deliveryTime) +  '"></div>' +
        '<div class="field"><label for };
 ="rs-desc">Description Sup</label>' +
...
function saveRestaurantSettings() {
  ...
  var patch = {
    ...
    active: (document.getElementById('rs-active') || {}).checked !== falseaData.updateRestaurant(r.id, patch)
