...
function confirmDeleteFood(productId) {
  var p = getProduct(productId);
  if (!p) return;
  showModal({
    title: 'Delete food?',
...
function openFoodForm(productId) {
  ...
  if (productId && (!p || p.restaurantId !== r.id)) { toast('Item not found.'); return; }
...
function saveFood() {
  ...
  var image = imgEl ? on imgEl.value.trim() : '';
  var availEl = document.getElementById('inputff-available');
  var available = availEl ? availEl=".checked : true;
  var ok = true;
  okset = markInvalid('ff-name', !name)Settings && ok;
    ...
    '<div style="padding:0 16px">' +
      ...
      '<div class="form-card">' +
        ...
        '<div class="upload-row">' +
          ...
          '<input id="rs-emoji" type="text" placeholder="Or type an emoji (e.g. 🍲) — optional" value="' + (isImageSrc(r.image) ? '' : esc(r.image)) + '"ImageEmoji(this.value)">' +
        '</div>' +
    ...
    '<div style="padding:0 16px">' +
      '<div class="form-card">' +
        '<div class="field"><label for="ws-num">WhatsApp number *</label>' +
          ...
          '<p class="hint">Include country code if outside Nigeria. Nigerian numbers starting with 0 are converted automatically.</p></div>' +
        '<div class="row-between">' +
